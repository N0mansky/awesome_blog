...
class ModelMetaclass(type):
    def __new__(cls, name, bases, attrs):
        ...
        for k in mappings.keys():
            attrs.pop(k)
        escaped_field = ['`{}`'.format(x) for x in fields]
        attrs['__mappings__'] = mappings
        attrs['__table__'] = tableName
        attrs['__primary_key__'] = primaryKey
        attrs['__fields__'] = fields
        attrs['__select__'] = 'select `{}`,{} from `{}`'.format(primaryKey, ', '.join(escaped_field), tableName)
        attrs['__insert__'] = 'insert into `{}` ({},`{}`) values ({})'.format(tableName, ', '.join(escaped_field),
                                                                              primaryKey,
                                                                              create_args_string(len(mappings)))
        placeholder = map(lambda x: '{}=?'.format(x), escaped_field)
        attrs['__update__'] = 'update `{}` set {} where `{}`=?'.format(tableName, ','.join(placeholder),
                                                                       escaped_field, primaryKey)
